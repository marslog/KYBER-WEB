      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-10 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 text-sm font-medium text-[var(--brand)] hover:underline shrink-0"
          >
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {RESOURCE_CARDS.map((resource) => (
            <Link
              key={resource.id}
              href={resource.href}
              className="group bg-white border border-[var(--border)] rounded-xl overflow-hidden hover:border-[var(--brand)] hover:shadow-[0_8px_24px_rgba(16,24,40,0.06)] transition-all"
            >
              {resource.image ? (
                <div className="relative h-32 border-b border-[var(--border)] bg-[var(--bg-muted)]">
                  <Image src={resource.image} alt={resource.title} fill className="object-cover object-top" sizes="25vw" />
                </div>
              ) : (
              ...
              <div className="p-4 flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <p className="text-[10px] font-medium uppercase tracking-wider text-[var(--text-muted)] mb-1">{resource.type}</p>
                  <h3 className="text-sm font-semibold leading-snug">{resource.title}</h3>
                </div>
                <ArrowRight className="w-3.5 h-3.5 text-[var(--text-muted)] group-hover:text-[var(--brand)] shrink-0 group-hover:translate-x-0.5 transition-all" />
              </div>
            </Link>
          ))}
        </div>
